...
Figure5 <- ggplot(market_share, aes(x = Year, y = yearly_growth_rate, group = type, fill = type)) + 
  geom_bar(stat = 'identity', size = 1, position = position_dodge(width = 0.9)) +
  labs(y = "Yearly Growth Rate", title = "Figure 5 Yearly Growth Rate of Market Share for China and LDCs") +
  guides(color = guide_legend(title = "")) +
  theme_bw() +
  theme(text = element_text(size = 10))
